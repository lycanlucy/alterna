...
public record MobVariant(ResourceLocation texture, String model, HolderSet<Biome> biomes) {
    public static final ResourceKey<Registry<MobVariant>> REGISTRY = ResourceKey.createRegistryKey(Alterna.id("mob_variant"));
    ...
    public static void bootstrap(BootstrapContext<MobVariant> context) {
        context.register(OCEAN_SALMON, new MobVariant(Alterna.id("textures/entity/fish/ocean_salmon.png"), OCEAN_SALMON_MODEL, context.lookup(Registries.BIOME).getOrThrow(BiomeTags.IS_OCEAN)));
        context.register(RIVER_SALMON, new MobVariant(Alterna.id("textures/entity/fish/river_salmon.png"), RIVER_SALMON_MODEL, context.lookup(Registries.BIOME).getOrThrow(BiomeTags.IS_RIVER)));
    }
    ...
    public static @Nullable MobVariant getFor(Entity entity) {
    ...
    public static Registry<MobVariant> getRegistry(Entity entity) {
        return entity.registryAccess().registryOrThrow(REGISTRY);
    }

    private static ResourceKey<MobVariant> key(String name) {
        return ResourceKey.create(REGISTRY, Alterna.id(name));
    }
}
