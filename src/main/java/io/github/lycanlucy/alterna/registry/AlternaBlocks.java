...
public class AlternaBlocks {
    ...
    private static DeferredBlock<Block> itemRack(String name, SoundType soundType, boolean fireImmune) {
        BlockBehaviour.Properties properties = BlockBehaviour.Properties.of().sound(soundType).strength(1.0f).noCollission().instrument(NoteBlockInstrument.BASS);
        return BLOCKS.register(name, () -> new ItemRackBlock(fireImmune ? properties : properties.ignitedByLava()));
    }
